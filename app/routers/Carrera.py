...
def router_crear_carrera(nombre: str, session: Session = Depends(get_session)):
    result = crear_carrera(session, nombre)
    if result == 400:
        raise HTTPException(
...
def router_delete_carrera(carrera_id: int, session: Session = Depends(get_session)):
    ...
    if result is None:
        raise HTTPException(
            status_code=404,
            detail=f"No se pudo actualizar: La carrera con ID {carrera_id} no se encontro",
        )
